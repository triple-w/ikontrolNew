...
class CommercialRemissionController extends Controller
{
    ...
    public function createFromQuote(Request $request, CommercialQuote $commercialQuote)
    {
        ...
        if ($commercialQuote->status !== CommercialQuote::STATUS_ACCEPTED) {
            throw ValidationException::withMessages(['commercial_quote_id' => 'Solo una cotizacion aceptada puede generar remisiones.']);
        }
    ...
    public function store(StoreCommercialRemissionRequest $request, CommercialQuoteCalculator $calculator, CommercialRemissionFolioService $folioService)
    {
        ...
        $remission = DB::transaction(function () use ($data, $client, $calculated, $request, $folioService) {
            $folio = $folioService->reserve((int) $client->users_id);
            $remission = CommercialRemission::create(array_merge(
                $this->remissionPayload($data, $client, $calculated, CommercialRemission::STATUS_DRAFT),
                $folio,
                [
            ...
            $this->recordStatus($remission, null, CommercialRemission::STATUS_DRAFT, (int) $request->user()->id, 'Remision creada.');
    ...
    private function validatedQuote(null|int|string $quoteId, int $ownerId): ?CommercialQuote
    {
        ...
        if (!$quote || $quote->status !== CommercialQuote::STATUS_ACCEPTED) {
            throw ValidationException::withMessages(['commercial_quote_id' => 'La cotizacion no pertenece al usuario autorizado o no esta aceptada.']);
        }
