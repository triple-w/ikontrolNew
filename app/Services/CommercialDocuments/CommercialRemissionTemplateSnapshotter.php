...
class CommercialRemissionTemplateSnapshotter
{
    public function ensureSnapshot(CommercialRemission $remission): void
    {
        if (!empty($remission->template_name_snapshot) || !empty($remission->template_options_snapshot)) {
            return;
        }

        $this->capture($remission, $remission->documentTemplate);
    }

    public function capture(CommercialRemission $remission, ?CommercialDocumentTemplate $template): void
    {
        $logoSnapshot = null;

        if ($template && $template->show_logo && $template->logo_path) {
            $logoSnapshot = $this->copyLogoSnapshot($remission, $template->logo_path);
        }
    ...
    private function copyLogoSnapshot(CommercialRemission $remission, string $logoPath): ?string
    {
        ...
        $relativeDir = 'private/commercial-documents/remissions/' . $remission->id;
